...
## External course providers, streaming, and Pinterest — what's real here

Every subject at every college/undergraduate/master's level (C1–M2) now includes a "More Courses" panel
(`external_courses` in `backend/syllabus/level_*.json`) with real, live search links into **Udemy, Coursera,
edX, MIT OpenCourseWare, Harvard Online Learning** (`pll.harvard.edu`), and **Class Central** (a real site that
aggregates free/audit courses from Coursera, edX, MIT, Harvard, and more in one search), plus a **Pinterest**
search for visual study notes. The World Cinema library links each movie to Netflix's and Prime Video's own
search pages (pre-existing) and now also to **JustWatch**, a real service that shows which platforms currently
carry a title, by region. Music & Instruments also links out to Pinterest for chord charts/diagrams per instrument.

**What this is not**: there is no way to embed a specific person's paid Udemy course library, or Netflix/Prime
Video's licensed catalog, inside a third-party app — those platforms have no public API for that, and doing it
would require the user's real login/DRM session, which this project does not have and should not fabricate.
So none of the above is a "connected account" — every link is a plain, unauthenticated search or catalog page on
the provider's own site, generated by `backend/scripts/generate_advanced_curriculum.py`,
`backend/scripts/augment_movies_streaming_links.py`, and `backend/scripts/generate_music_data.py`. If you have
real API credentials (e.g. a Udemy Affiliate/Business API key, a Coursera or edX partner API key, or a Pinterest
OAuth app token) and want genuine account-linked integration instead, that would need those credentials supplied
as environment variables/secrets and a new API client per provider — a materially bigger change than the
search-link integration shipped here.
...
Each learner now has a persistent, explainable concept knowledge map derived
from the curriculum's authored lesson metadata. Practice answers and lesson
mini-checks update a Bayesian-style mastery estimate, schedule spaced reviews,
track repeated misconceptions, select an adaptive difficulty from foundation
through stretch, and recommend the next lesson with a learner-visible reason.
The Subjects screen shows overall mastery, reviews due, the concept map,
misconception patterns, and a direct link to the recommended lesson. State is
stored in the `concept_mastery` database table and follows the learner profile
across browsers and devices.

### Curriculum minimums, visible lessons, dashboard reliability, and film thumbnails

Every subject now meets the published minimum for its level: 20 lessons at
Levels 1-3, 30 at 4-6, 40 at 7-8, 50 at 9-10, 70 at C1-C2, 100 at UG1-UG4,
and 120 at M1-M2. C1 and C2 contain 52 subjects, compared with 26 at Level
10. Critical Thinking, Environmental Science, World Politics, World
Religions, Civics, Health Education, and Business Studies each contain at
least 100 adult lessons (120 at master's level).

The Subjects screen renders the actual `subject.lessons` collection for Math,
English, and every other subject. It includes search, 20-item paging, lesson
details, objectives, activities, citations, and individual completion
tracking. The professional database schema is created on first local startup,
preventing a fresh install from returning 500 on its first dashboard request.

The BFI top-200 film records also include 200 original local SVG catalogue
thumbnails generated by `backend/scripts/generate_movie_thumbnails.py`.
`backend/scripts/audit_resource_links.py` structurally validates every
curriculum resource URL and performs a bounded live endpoint audit without
flooding course providers with tens of thousands of duplicate searches.

### EduAI_Pro adult professional workspace

The new **Professional** workspace is the adult product surface for Level 1
through Master's M2. It provides research projects and notes, local-document
import and semantic-style ranked search, bibliography/BibTeX export,
diagnostic and configurable assessments, assignments/submissions, portfolio
and CPD records, career pathways, organizations, cohorts, attendance,
discussions, SCORM export, xAPI statement templates, and LTI configuration.
The adult AI Tutor supports direct, Socratic, worked-example, and competing
perspectives modes. Its answers are grounded in approved curriculum resources,
return citations, retain conversation history, and enforce a configurable
monthly API-spend ceiling.

Production persistence uses SQLAlchemy with PostgreSQL, UUID identifiers,
constraints, transactions, soft deletion, Alembic migrations, and audit
events. Accounts, progress, activity, homework, reading logs, screen time,
attendance, curriculum edits, research, assessments, AI conversations, and
professional records no longer use JSON as runtime storage. The original JSON
curriculum remains a read-only seed/import format so the large existing
catalogue can be migrated without losing source history.
...
Docker Compose starts PostgreSQL, Redis, the API, and the frontend. The API
container applies Alembic migrations before it starts. For an existing
installation, import the legacy JSON exactly once (the importer is
idempotent):
...
Set `DATABASE_URL` to a PostgreSQL URL in production. If it is omitted, local
development and tests use `backend/data/eduai_pro.db` as a SQLite fallback.
Set `AI_MONTHLY_BUDGET_USD` to cap model spend. Moodle, Canvas, Teams, and
other institutional deployments still require credentials and registration
from those platforms; this repository supplies standards-based SCORM/xAPI/LTI
entry points, not fabricated connected accounts.

### Open courses and local owned-content library

The **Resource Tab** includes a unified catalogue for MIT OpenCourseWare,
Harvard Professional & Lifelong Learning, edX, Coursera, Udemy, OpenLearn,
Saylor Academy, Khan Academy, and free YouTube course searches. The interface
states each provider's access model because some platforms mix free or audited
learning with paid certificates and courses.

It can also scan a folder on the computer running EduAi_Pro for owned books
(`.pdf`, `.docx`, `.epub`, `.txt`), video, audio, and pictures. Source files
are not copied. The local index stores only their original paths, metadata,
summaries, and curriculum associations. Clicking an item streams the original
file from its local path. Books are analysed locally with a transparent
TF-IDF-style NLP relevance score and matched to Level 1 through M2
subjects/topics; document text is not sent to an external service.

### Top-2000 museum additions

The Virtual Museum includes a **Top Collection Additions** gallery with
exactly 2,000 Cleveland Museum of Art open-access objects. Its deterministic
generator exhausts unique titles from the first 2,000 bundled source records,
then continues in source order to complete the requested total. It skips
duplicates and retains each preview image, official museum record, discovery
links, source identifier, and source rank. Run
`python backend/scripts/add_top_2000_museum_missing.py` to rebuild it
idempotently.

### Chart-verified songs and all-time films

The Song Centre includes a 500-song batch sourced from Kworb's current
all-time YouTube music-video table. Every selected video had at least
700,000,000 observed views at import time; the bundled snapshot retains its
chart rank, exact observed count, video id, source link, and check date.
Re-run `python backend/scripts/add_700m_chart_songs.py` to refresh the batch.
View totals change continuously and chart popularity is not presented as an
artistic-quality rating.

The Movies Library includes the first 200 distinct films from the official
BFI *Sight and Sound* Greatest Films of All Time poll page. Each film has its
BFI rank and record, preview image, plus discovery searches for IMDb, TMDB,
Rotten Tomatoes, Letterboxd, Wikipedia, JustWatch, Netflix, Prime Video,
Apple TV, YouTube Movies, and BFI Player. Streaming links are searches rather
than claims of subscription availability, which varies by country and date.

This repository now contains the populated Level 1 through Master's M2
curriculum and adult professional workspace. Existing authored lessons and
verified resources are preserved; the deterministic minimum-enforcement
script adds structured lessons only where a level is below its declared floor.
...
- Backend tests (pytest) and frontend tests (Vitest + Testing Library), plus lint and production-build checks.
...
Provider-connected paid accounts still require the provider's credentials and
partner approval; the bundled catalogue uses public search and open-course
links and does not pretend to bypass subscriptions.
...
This creates the backend virtualenv and installs its dependencies, applies database migrations, imports legacy curriculum, runs `npm install` for the frontend, and writes `start_EduAi_Pro.sh` or `start_EduAi_Pro.bat`. On Windows it creates a real **EduAi_Pro.lnk** shortcut through the built-in Windows scripting interface on both the normal Desktop and every configured OneDrive Desktop. Re-running it safely refreshes the launcher and shortcuts.
