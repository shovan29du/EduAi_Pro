...
def test_grade4_has_full_depth_real_chart_coverage():
    """Depth pass: every single lesson in every subject at Grade 4 should
    have a genuine, verifiable data_table/graph/formulae."""
    data = json.loads((SYLLABUS_DIR / "grade4.json").read_text(encoding="utf-8"))
    total = 0
    covered = 0
    for subject_name, subject in data["subjects"].items():
        for lesson in subject["lessons"]:
            ...
            if has_content:
                ...
                if lesson.get("data_table"):
                    ...
                    for row in lesson["data_table"]["rows"]:
                        assert len(row) == len(lesson["data_table"]["headers"])
    assert total == 540
    assert covered == total, f"Grade 4 depth coverage incomplete: {covered}/{total}"
...
def test_grade6_has_full_depth_real_chart_coverage():
    """Depth pass: every single lesson in every subject at Grade 6 should
    have a genuine, verifiable data_table/graph/formulae. Grade 6 adds a
    19th subject (Critical Thinking) vs Grades 1-5."""
    data = json.loads((SYLLABUS_DIR / "grade6.json").read_text(encoding="utf-8"))
    total = 0
    covered = 0
    for subject_name, subject in data["subjects"].items():
        for lesson in subject["lessons"]:
            ...
            if has_content:
                ...
                if lesson.get("data_table"):
                    ...
                    for row in lesson["data_table"]["rows"]:
                        assert len(row) == len(lesson["data_table"]["headers"])
    assert total == 570
    assert covered == total, f"Grade 6 depth coverage incomplete: {covered}/{total}"

    mongol = {l["id"]: l for l in data["subjects"]["World History"]["lessons"]}["world-history-g6-l16"]
    assert ["Founded by", "Genghis Khan"] in mongol["data_table"]["rows"]


def test_grade7_has_full_depth_real_chart_coverage():
    """Depth pass: every single lesson in every subject at Grade 7 should
    have a genuine, verifiable data_table/graph/formulae. Grade 7 has 40
    lessons per subject across 19 subjects (760 total)."""
    data = json.loads((SYLLABUS_DIR / "grade7.json").read_text(encoding="utf-8"))
    total = 0
    covered = 0
    for subject_name, subject in data["subjects"].items():
        for lesson in subject["lessons"]:
            ...
            if has_content:
                ...
                if lesson.get("data_table"):
                    ...
                    for row in lesson["data_table"]["rows"]:
                        assert len(row) == len(lesson["data_table"]["headers"])
    assert total == 760
    assert covered == total, f"Grade 7 depth coverage incomplete: {covered}/{total}"
...
def test_grade9_has_full_depth_real_chart_coverage():
    """Depth pass: every single lesson in every subject at Grade 9 should
    have a genuine, verifiable data_table/graph/formulae. Grade 9 has 50
    lessons per subject across 26 subjects (1300 total)."""
    data = json.loads((SYLLABUS_DIR / "grade9.json").read_text(encoding="utf-8"))
    total = 0
    covered = 0
    for subject_name, subject in data["subjects"].items():
        for lesson in subject["lessons"]:
            ...
            if has_content:
                ...
                if lesson.get("data_table"):
                    ...
                    for row in lesson["data_table"]["rows"]:
                        assert len(row) == len(lesson["data_table"]["headers"])
    assert total == 1300
    assert covered == total, f"Grade 9 depth coverage incomplete: {covered}/{total}"

    newtons_second_law = {l["id"]: l for l in data["subjects"]["Physics"]["lessons"]}["physics-g9-l11"]
    assert "F = ma" in newtons_second_law["formulae"]
