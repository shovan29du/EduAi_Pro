#!/usr/bin/env python3
"""Depth pass, C2 Business Studies: fill in real, hand-checked
data_table content for the 99 C2 Business Studies lessons not
covered by the earlier breadth-first batch. Brings C2 Business
Studies to full 100/100 coverage.
...
"Applied Research Seminar" (l81-l100). l3 (topic 3, Entrepreneurship)
...
Re-run after editing:
    python3 backend/scripts/add_c2_business_studies_completion.py
"""
...
for offset in MODE_TO_OFFSET.values():
    for i, topic_table in enumerate(TOPIC_TABLES):
        lesson_n = offset + i + 1
        CHARTS[f"business-studies-c2-l{lesson_n}"] = {"data_table": topic_table}
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_c2.json Business Studies: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} C2 Business Studies lessons (completing 100/100).")
