#!/usr/bin/env python3
"""Depth pass, C2 World Religions: fill in real, hand-checked
data_table content for the 99 C2 World Religions lessons not covered
by the earlier breadth-first batch. Brings C2 World Religions to full
100/100 coverage.
...
"Applied Research Seminar" (l81-l100). l3 (topic 3, Hindu Traditions)
...
Re-run after editing:
    python3 backend/scripts/add_c2_world_religions_completion.py
"""
...
for offset in MODE_TO_OFFSET.values():
    for i, topic_table in enumerate(TOPIC_TABLES):
        lesson_n = offset + i + 1
        CHARTS[f"world-religions-c2-l{lesson_n}"] = {"data_table": topic_table}
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_c2.json World Religions: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} C2 World Religions lessons (completing 100/100).")
