#!/usr/bin/env python3
"""Depth pass, M1 Business Studies: fill in real, hand-checked
data_table content for the 119 M1 Business Studies lessons not
covered by the earlier breadth-first batch. Brings M1 Business
Studies to full 120/120 coverage.
...
Re-run after editing:
    python3 backend/scripts/add_m1_business_studies_completion.py
"""
...
# all 6 modes (Conceptual Foundations, Worked Analysis, Evidence and
# Data, Comparative Case Study, Applied Research Seminar, Independent
# Capstone).
...
for offset in MODE_TO_OFFSET.values():
    for i, topic_table in enumerate(TOPIC_TABLES):
        lesson_n = offset + i + 1
        CHARTS[f"business-studies-m1-l{lesson_n}"] = {"data_table": topic_table}
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_m1.json Business Studies: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} M1 Business Studies lessons (completing 120/120).")
