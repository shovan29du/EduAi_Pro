#!/usr/bin/env python3
"""Depth pass, M2 Civics: fill in real, hand-checked data_table
content for the M2 Civics lessons not covered by the earlier
breadth-first batch. Brings M2 Civics to full 120/120 coverage.

Structure (same topic-block pattern as M1 Civics): 20 topics, each
covered in 6 modes at offsets of 20 lessons apart -- "Conceptual
...
3, Rule of Law) was already completed by an earlier breadth-first
...
Re-run after editing:
    python3 backend/scripts/add_m2_civics_completion.py
"""
...
for offset in MODE_TO_OFFSET.values():
    for i, topic_table in enumerate(TOPIC_TABLES):
        lesson_n = offset + i + 1
        CHARTS[f"civics-m2-l{lesson_n}"] = {"data_table": topic_table}
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_m2.json Civics: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} M2 Civics lessons (completing 120/120).")
