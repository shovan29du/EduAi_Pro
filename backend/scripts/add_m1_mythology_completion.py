#!/usr/bin/env python3
"""Depth pass, M1 Mythology: fill in real, hand-checked data_table
content for the 15 M1 Mythology lessons not covered by the earlier
breadth-first batch. Brings M1 Mythology to full 16/16 coverage.

Unlike most M1 subjects, Mythology has only 16 lessons, one per
...
Re-run after editing:
    python3 backend/scripts/add_m1_mythology_completion.py
"""
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_m1.json Mythology: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} M1 Mythology lessons (completing 16/16).")
