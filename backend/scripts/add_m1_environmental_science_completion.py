#!/usr/bin/env python3
"""Depth pass, M1 Environmental Science: fill in real, hand-checked
data_table content for the 119 M1 Environmental Science lessons not
covered by the earlier breadth-first batch. Brings M1 Environmental
...
earlier breadth-first batch with a data_table matching this same
topic table, so it is reused here as-is (idempotent no-op for l3).

Idempotent: only fills in fields that aren't already set.

Re-run after editing:
    python3 backend/scripts/add_m1_environmental_science_completion.py
"""
...
for offset in MODE_TO_OFFSET.values():
    for i, topic_table in enumerate(TOPIC_TABLES):
        lesson_n = offset + i + 1
        CHARTS[f"environmental-science-m1-l{lesson_n}"] = {"data_table": topic_table}
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_m1.json Environmental Science: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} M1 Environmental Science lessons (completing 120/120).")
