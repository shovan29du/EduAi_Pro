#!/usr/bin/env python3
"""Depth pass, M1 World Religions: fill in real, hand-checked
data_table content for the 119 M1 World Religions lessons not covered
by the earlier breadth-first batch. Brings M1 World Religions to full
120/120 coverage.
...
(l101-l120). l3 (topic 3, Hindu Traditions) was already completed by
...
Re-run after editing:
    python3 backend/scripts/add_m1_world_religions_completion.py
"""
...
for offset in MODE_TO_OFFSET.values():
    for i, topic_table in enumerate(TOPIC_TABLES):
        lesson_n = offset + i + 1
        CHARTS[f"world-religions-m1-l{lesson_n}"] = {"data_table": topic_table}
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_m1.json World Religions: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} M1 World Religions lessons (completing 120/120).")
