#!/usr/bin/env python3
"""Depth pass, M1 Civics: fill in real, hand-checked data_table
content for the 119 M1 Civics lessons not covered by the earlier
breadth-first batch. Brings M1 Civics to full 120/120 coverage.
...
(l101-l120). l3 (topic 3, Rule of Law) was already completed by an
...
Re-run after editing:
    python3 backend/scripts/add_m1_civics_completion.py
"""
...
for offset in MODE_TO_OFFSET.values():
    for i, topic_table in enumerate(TOPIC_TABLES):
        lesson_n = offset + i + 1
        CHARTS[f"civics-m1-l{lesson_n}"] = {"data_table": topic_table}
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_m1.json Civics: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} M1 Civics lessons (completing 120/120).")
