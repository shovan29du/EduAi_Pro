#!/usr/bin/env python3
"""Depth pass, M2 World Politics: fill in real, hand-checked
data_table content for the M2 World Politics lessons not covered by
the earlier breadth-first batch. Brings M2 World Politics to full
120/120 coverage.

Structure (same topic-block pattern as M1 World Politics): 20 topics,
...
3, Political Ideologies) was already completed by an earlier
breadth-first batch with a data_table matching this same topic table,
so it is reused here as-is (idempotent no-op for l3).

Idempotent: only fills in fields that aren't already set.

Re-run after editing:
    python3 backend/scripts/add_m2_world_politics_completion.py
"""
...
for offset in MODE_TO_OFFSET.values():
    for i, topic_table in enumerate(TOPIC_TABLES):
        lesson_n = offset + i + 1
        CHARTS[f"world-politics-m2-l{lesson_n}"] = {"data_table": topic_table}
...
def main() -> None:
    ...
    if missing:
        raise SystemExit(f"Lesson ids not found in level_m2.json World Politics: {missing}")
    ...
    print(f"Added {updated} fields across {len(CHARTS)} M2 World Politics lessons (completing 120/120).")
